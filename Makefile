...
.PHONY: help audit fm pipeline verify-logs lint format test clean
...
help:
	...
	@echo "  verify-logs  - Parse latest pipeline log for JSON tool summaries"
...
verify-logs: ## Scan latest logs/pipeline/*.run.log for JSON tool summaries (portable)
	@set -e; \
	latest=$$(ls -1t logs/pipeline | head -n 1); \
	echo "Latest log: logs/pipeline/$${latest}"; \
	echo "[JSON summaries]"; \
	awk '\''match($$0,/\{"tool":[^}]*\}/){print NR ":" substr($$0, RSTART, RLENGTH)}'\'' "logs/pipeline/$${latest}" || true; \
	echo "[Tools seen]"; \
	grep -E "academy_docs_scraper\.py|analyze_creatio_structure\.py|comprehensive_solutions_scraper\.py|authenticated_knowledge_hub_scraper\.py" -n "logs/pipeline/$${latest}" || true
